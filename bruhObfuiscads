...
do
	function utility:Create(instance, properties, children)
		...
		for i, module in pairs(children or {}) do module.Parent = object end
		return object
	end
	...
	function utility:Sort(pattern, values)
		...
		for i, value in pairs(values) do
			if tostring(value):lower():find(pattern) then table.insert(new, value) end
		end
		return new
	end
	...
	function utility:InitializeKeybind()
		...
		input.InputBegan:Connect(function(key)
			if self.keybinds[key.KeyCode] then
				for i, bind in pairs(self.keybinds[key.KeyCode]) do bind() end
			end
		end)
		input.InputEnded:Connect(function(key)
			if key.UserInputType == Enum.UserInputType.MouseButton1 then for i, callback in pairs(self.ended) do callback() end end
		end)
	end
		...
		return {
			UnBind = function()
				for i, bind in pairs(self.keybinds[key]) do
					if bind == callback then table.remove(self.keybinds[key], i) end
				end
			end
		}
	end
	...
	function utility:KeyPressed()
		-- yield until next key is pressed
		local key = input.InputBegan:Wait()
		while key.UserInputType ~= Enum.UserInputType.Keyboard do key = input.InputBegan:Wait() end
		wait()
	...
	function utility:DraggingEnded(callback) table.insert(self.ended, callback) end
end
-- classes
local library = { MrJack = "MrJackIsCool" }
-- main
local page = {}
local section = {}
do
	...
	function library.new(title)
		local container = utility:Create("ScreenGui", { Name = RandomName(math.random(8, 20)) .. "GuiMrJack", Parent = game:GetService"CoreGui" }, {
		...
		if syn and syn.protect_gui then syn.protect_gui(container)
		elseif get_hidden_gui then get_hidden_gui(container) end
		utility:InitializeKeybind()
		utility:DraggingEnabled(container.Main.TopBar, container.Main)
		return setmetatable({ container = container, pagesContainer = container.Main.Pages.Pages_Container, pages = {} }, library)
	end
	...
	function library:addPage(...)
		...
		button.MouseButton1Click:Connect(function() self:SelectPage(page, true) end)
		return page
	end
		...
		for property, objects in pairs(objects[theme]) do
			for i, object in pairs(objects) do
				if not object.Parent or object.Name == "Button" and object.Parent.Name == "ColorPicker" then
					-- i can do this because weak tables :D
					objects[i] = nil
				else object[property] = color3 end
			end
		end
	end
	...
	function section:addButton(title, callback)
		...
		button.MouseButton1Click:Connect(function()
			...
			if callback then callback(function(...) self:updateButton(button, ...) end) end
			debounce = false
		end)
		return button
	end
	...
	function section:addTextbox(title, default, callback)
		...
		input:GetPropertyChangedSignal"Text":Connect(function()
			...
			if callback then
				callback(input.Text, nil, function(...) self:updateTextbox(textbox, ...) end)
			end
		end)
		input.FocusLost:Connect(function()
			...
			if callback then
				callback(input.Text, true, function(...) self:updateTextbox(textbox, ...) end)
			end
		end)
		return textbox
	end
	...
	function section:addColorPicker(title, default, callback)
		...
		local callback = function(value)
			if callback then
				callback(value, function(...) self:updateColorPicker(colorpicker, ...) end)
			end
		end
		...
		for i, container in pairs(tab.Container.Inputs:GetChildren()) do
			-- i know what you are about to say, so shut up
	...
	function section:addSlider(title, default, min, max, callback)
		...
		local callback = function(value)
			if callback then
				callback(value, function(...) self:updateSlider(slider, ...) end)
			end
		end
	...
	function library:SelectPage(page, toggle)
		...
		if toggle then
			...
			page:Resize()
			for i, section in pairs(page.sections) do section.container.Parent.ImageTransparency = 0 end
	...
	function section:Resize(smooth)
		...
		-- offset
		for i, module in pairs(self.modules) do size = size + module.AbsoluteSize.Y + padding end
		if smooth then
	...
	function section:getModule(info)
		...
		for i, module in pairs(self.modules) do
			if (module:FindFirstChild"Title" or module:FindFirstChild("TextBox", true)).Text == info then return module end
		end
	...
	function section:updateColorPicker(colorpicker, title, color)
		...
		if type(color) == "table" then
			-- roblox is literally retarded x2
	...
	function section:updateDropdown(dropdown, title, list, callback)
		...
		for i, button in pairs(dropdown.List.Frame:GetChildren()) do if button:IsA"ImageButton" then button:Destroy() end end
		for i, value in pairs(list or {}) do
			...
			button.MouseButton1Click:Connect(function()
				if callback then
					callback(value, function(...) self:updateDropdown(dropdown, ...) end)
				end
